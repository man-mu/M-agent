	...
	void emitsVisibleSkippedEventWhenNoDocumentsRetrieved() {
		...
		StepVerifier.create(node.run(state))
			...
			.assertNext(event -> {
				...
				assertThat(event.payload()).isEqualTo("No user-upload RAG context was retrieved.");
			})
			.verifyComplete();
	}

	private ResearchState stateWithQueryAndSession(String query, String sessionId) {
		ResearchState state = ResearchState.from(new ResearchRequest(query, "thread-1", 2), sessionId);
		ResearchPlan plan = new ResearchPlan("Test", true, "Test plan",
				List.of(new ResearchStep("Step", "Desc", false, StepType.RESEARCH, null, "pending")));
		state.plan(plan);
		return state;
	}
