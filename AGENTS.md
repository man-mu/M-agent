...
- 执行实现计划时优先采用当前会话内的逐步执行方式，不额外引入多子代理协作流程，除非用户明确要求。
- 项目计划文档保存在 `docs/superpowers/plans/` 目录下。
...
- 以下模块已确认现有实现足够，不再继续向 `deepresearch-main` 参考项目做复杂能力对齐：搜索能力（多搜索引擎/智能选择）、智能体系统（问题分类/领域专项 Agent/反思机制）、代码执行（Python 沙箱/Docker 执行）、RAG 增强（混合检索/HyDe/RRF 融合/批量上传）。
